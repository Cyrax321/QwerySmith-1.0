...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig3_clause_f1_scores.png}
\caption{\textbf{Figure 3:} AST clause detection F1 scores across standard SQL syntactic constructs, highlighting fine-tuned model gains on complex relational clauses (JOIN, GROUP BY, HAVING).}
\label{fig:fig3}
\end{figure}
...
Figure~\ref{fig:fig3} contrasts clause-level F1 scores, while Figure~\ref{fig:fig7} depicts the $2\times4$ grid of clause-level confusion matrices for QwerySmith 1.1. Table~\ref{tab:diagnostic_clause_matrix} compiles the comprehensive diagnostic metrics, including Matthews Correlation Coefficient (MCC), Specificity, and Balanced Accuracy.
...
\subsection{Complexity Tier Stratification}
Figure~\ref{fig:fig9} and Table~\ref{tab:complexity_matrix} illustrate execution accuracy stratified across academic SQL complexity tiers: Simple (Projection/Filter), Moderate (Aggregation/Sorting), Complex (Multi-Table Joins), and Advanced (Nested Subqueries / Set Operations). QwerySmith sustains competitive execution accuracy across all levels of structural nesting.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig9_complexity_heatmap.png}
\caption{\textbf{Figure 9:} Execution accuracy stratification across SQL complexity tiers (Simple, Moderate, Complex, Advanced) for all four evaluated benchmark splits.}
\label{fig:fig9}
\end{figure}
...
\midrule
In Dist & 92.3\% & 87.5\% & 84.6\% & 80.0\% \\
Gretel Test & 68.4\% & 57.1\% & 48.9\% & 38.5\% \\
Heldout Sqale & 55.6\% & 46.2\% & 41.7\% & 33.3\% \\
Heldout Large Schema & 28.6\% & 18.2\% & 14.3\% & 10.5\% \\
\bottomrule
\end{tabular}
...
\subsection{Error Taxonomy \& Failure Modes}
Figure~\ref{fig:fig10} and Table~\ref{tab:error_taxonomy} decompose non-passing predictions into explicit failure modes: Syntax Error, Join Error, Aggregation Error, Predicate Error, and Semantic Row Mismatch. Fine-tuning reduces Syntax Errors by 57\% and Join Errors by 42\% relative to the 3-shot base model.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig10_error_taxonomy_matrix.png}
\caption{\textbf{Figure 10:} Comparative error taxonomy and failure mode breakdown, demonstrating substantial reduction in syntax and join invalidities under QwerySmith 1.1.}
\label{fig:fig10}
\end{figure}
...
\midrule
Syntax Error & 64 & 44 & \textbf{19} \\
Join Error & 48 & 50 & \textbf{29} \\
Aggregation Error & 42 & 42 & \textbf{23} \\
Predicate Error & 58 & 63 & \textbf{38} \\
Semantic Row Mismatch & 123 & 136 & \textbf{112} \\
\bottomrule
\end{tabular}
