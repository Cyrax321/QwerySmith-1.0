...
- **Sub-Millisecond Retrieval (<1ms)**: Engineered as a zero-dependency in-process cognitive state bus using SQLite WAL mode with dual B-Tree indexing on session turns (`O(log N)` lookup in ~30µs) and compiled FTS5 BM25 search in ~0.38ms. No cloud vector database or network latency.
