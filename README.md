...
[![Model family](https://img.shields.io/badge/%F0%9F%A4%97%20Models-QwerySmith%20family-blue)](https://huggingface.co/Cyrax321)
![Python 3.11+](https://img.shields.io/badge/python-3.11%2B-blue)
![License](https://img.shields.io/badge/license-Apache--2.0-green)

Open text-to-SQL models that answer from retrieved evidence with
machine-verifiable citations - plus the harness that trains, evaluates,
and releases them. Weights on [Hugging Face](https://huggingface.co/Cyrax321);
family document in [MODELS.md](MODELS.md).

## The models

| Model | Base | What it does | Weights |
|---|---|---|---|
| **QwerySmith-2.0** | Qwen3-8B (QLoRA) | Answers database questions from retrieved evidence: emits SQL + a cited answer, or refuses when the evidence lacks the answer. 3-seed release; median-EX seed is the canonical repo. | [Cyrax321/QwerySmith-2.0](https://huggingface.co/Cyrax321/QwerySmith-2.0) |
| QwerySmith-2.0-seed{1,2,3} | Qwen3-8B | The three release seeds, each with its full train record (GPU, hashes, loss curve) and measured results. | [seed1](https://huggingface.co/Cyrax321/QwerySmith-2.0-seed1) / [seed2](https://huggingface.co/Cyrax321/QwerySmith-2.0-seed2) / [seed3](https://huggingface.co/Cyrax321/QwerySmith-2.0-seed3) |
| QwerySmith-1.1 | Qwen3-4B | Curriculum fine-tune; its external evaluation (below base on unseen data) is the finding that defines 2.0: fine-tuning teaches behaviour, not facts. | [Cyrax321/QwerySmith-1.1](https://huggingface.co/Cyrax321/QwerySmith-1.1) |
| QwerySmith-1.0 | Qwen3-4B | First curriculum fine-tune; superseded. | [Cyrax321/QwerySmith-1.0](https://huggingface.co/Cyrax321/QwerySmith-1.0) |

Every 2.0 release ships with the pinned training recipe, the measured
evaluation table, hardware manifest, loss figures, and stated limitations
- generated by the harness in this repo, never hand-written. A failed gate
is published as "provenance, not a recommended model".

## What the models do

```text
question --+--> retrieval (frozen evidence packs) --+--> QwerySmith 2.0
           |                                             |
   schema --+---------------------------------------------+--> SQL: one SELECT
                                                        |
                                                        +--> ANSWER: text with [table:pk] citations
                                                        |
                                                        +--> REFUSAL: when evidence lacks the answer
```

Facts always come from retrieval - the adapters store no data. The
fine-tune shapes behaviour only: the output contract, terminology, and
calibrated refusal. The 1.1 evaluation proved the alternative (teaching
facts) does not survive contact with unseen data.

## The harness

The harness turns any relational database with history into that system,
reproducibly: schema loading, question-set validation with mechanical
holdout enforcement, RAFT-style training triples, QLoRA training, a
four-system evaluation on identical questions and identical evidence, a
pre-registered gate, failure folders, and one-command release to
Hugging Face. Same pipeline, config changes only, per dataset - proven
automatically on a second differently-shaped database.

**Status:** harness complete, lint-clean, 110 tests green, CI on every
push. Publish path live. Awaiting Olist raw data for the first full
dataset run - no result numbers are claimed here; results land in
`runs/<dataset>/` as reports generated from measured output only, and the
gate verdict (whatever it turns out to be) is stated by code, not prose.
...
# 2. try the released model (adapter on the base)
#    pip install peft transformers
python -c "
from peft import PeftModel
from transformers import AutoModelForCausalLM, AutoTokenizer
base = AutoModelForCausalLM.from_pretrained('Qwen/Qwen3-8B', device_map='auto')
model = PeftModel.from_pretrained(base, 'Cyrax321/QwerySmith-2.0')
print('loaded - see MODELS.md for the prompt contract')"

# 3. prove the harness on the bundled toy shape (no data download needed)
uv run pytest tests_v3 -q                         # 110 green
uv run pytest tests_v3/test_reusability.py -q      # the config-only proof

# 4. first real run: drop the 9 Olist CSVs into datasets/olist/raw/, then
